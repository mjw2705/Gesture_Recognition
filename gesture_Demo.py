...
gesture_session = onnxruntime.InferenceSession('models/gesture_model2.onnx', None)
...
out = cv2.VideoWriter('gesture.avi', fourcc, 30, (frame_w, frame_h))
...
while cap.isOpened():
    ...
    if results.face_landmarks is None:
        ...
        sx, sy, ex, ey = utils.box_pos((face_sp[0], face_sp[1], face_ep[0], face_ep[1]), frame_w, frame_h)
        ...
        if results.left_hand_landmarks:
            ...
            joint, abs_joint = utils.get_landmark(landmarks, image)
            label = 'Left'  
        ...
        if joint is not None:
            ...
            # swipe 알고리즘
            if action == 'palm':
                # cropped 이미지에서 상대 좌표 구하기
                roi_width = ex - sx
                roi_height = ey - sy
                roi_x = hand_center_x - sx
                roi_y = hand_center_y - sy

                # 상대 좌표를 cropped 이미지에서 비율로 바꿈
                scaled_x = float(roi_x / roi_width)
                scaled_y = float(roi_y / roi_height)
    ...
    cv2.putText(image, f"FPS : {fps:.2f}", (10, 20), cv2.FONT_HERSHEY_TRIPLEX, 0.7, (255, 0, 0), 1)
